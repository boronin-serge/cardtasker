...
class CustomRecyclerView @JvmOverloads constructor(
    ...
    private fun initCustomProperties(attrs: AttributeSet) {
        context.withStyledAttributes(attrs, R.styleable.CustomRecyclerView) {
            val tlRadius = readFloat(R.styleable.CustomRecyclerView_topLeftCornerRadius)
            val trRadius = readFloat(R.styleable.CustomRecyclerView_topRightCornerRadius)
            val blRadius = readFloat(R.styleable.CustomRecyclerView_bottomLeftCornerRadius)
            val brRadius = readFloat(R.styleable.CustomRecyclerView_bottomRightCornerRadius)
    ...
    private fun TypedArray.readFloat(resId: Int): Float {
        return getDimensionPixelSize(resId, DEFAULT_INT).toFloat()
    }
